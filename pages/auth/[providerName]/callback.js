  ...
  const createUseInStrapi = async (qParams) => {
    ...
    try {
      const firstRequest = await axios({
        method: "get",
        url: `https://pimster-feature-backend.herokuapp.com/api/auth/${providerName}/callback/?${qParams}`,
      })
      console.log(firstRequest)
      if (firstRequest.status === 200) {
        const newStrapiUser = await axios({
          method: "get",
          url: `https://pimster-feature-backend.herokuapp.com/api/auth/${providerName}/callback/?access_token=${access_token}`,
        })
